...
(function () {
  ...
  var formatStatus = function formatStatus(st) {
    ...
    switch (st) {
      case 0:
        return '<div class="badge bg-warning text-dark fw-light">Archive</div>';

      case 1:
        return '<div class="badge bg-primary fw-light">Active</div>';

      case 3:
        return '<div class="badge bg-secondary fw-light">Experimental</div>';

      default:
        return '<div class="badge bg-secondary fw-light">Drafts</div>';
    }
  };
  var formatApplication = function formatApplication(application) {
    var html = '';
    if (Array.isArray(application)) application.forEach(function (val) {
    ...
    addProduct: function addProduct(e) {
      ...
      _this.listeners.modalSuccessBtnFunc = function (e) {
        ...
        fetch('https://api-v1.kenzap.cloud/', {
          ...
          if (response.success) {
            window.location.href = link("/medication-edit/?id=".concat(response.product.id));
          } else {
            parseApiError(response);
          }
